...
async function runTests() {
  ...
  devStates.set(testDevId, {
    ...
    history: [
      { time: Date.now() - 3600000 * 2, level: 80, isCharging: false }
    ]
  });

  mainHook.checkHourlyBattery();
  const updatedDev = devStates.get(testDevId);
  assert(updatedDev.history.length >= 2, 'checkHourlyBattery must add an hourly record to history');
  const lastRecorded = updatedDev.history[updatedDev.history.length - 1];
  assert.strictEqual(lastRecorded.level, 75, 'Hourly checkpoint must record current battery level');
  assert.strictEqual(lastRecorded.isCharging, false, 'Hourly checkpoint must record current charging state');
  assert.strictEqual(lastRecorded.isHourlyCheck, true, 'Hourly checkpoint must mark isHourlyCheck: true');

  // Test updateTrayBattery hourly interval recording
  if (typeof mainHook.updateTrayBattery === 'function') {
    const t0 = Date.now() - 3600000;
    devStates.set('tray-test-mouse', {
      level: 60,
      isCharging: false,
      history: [{ time: t0, level: 60, isCharging: false }]
    });
    mainHook.updateTrayBattery('tray-test-mouse', 60, false);
    const trayState = devStates.get('tray-test-mouse');
